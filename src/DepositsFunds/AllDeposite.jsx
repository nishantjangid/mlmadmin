...
import { approvedeposite, getdepositedata, rejectDeposite } from "../ApiHelpers";
...
function AllDeposite() {
  ...
  const imageTemplate = (data) => (
    <a href={IMAGE_URL + data.image}  target="_blank">Click Here</a>
  );
  const statusTemplate = (row) => (
    <span>
      {row?.status == 0 && (
        <p style={{ color: "#d34f1d", fontWeight: "bold" }}>Pending</p>
      )}
      {row?.status == 1 && (
  ...
  const handleApprove = async (depositId) => {
    try{
      setLoadings(true);
      let result = await approvedeposite({depositId});
      setLoadings(false);
  ...
  const handleReject = async (depositId) => {
    try{
      setLoadings(true);
      let result = await rejectDeposite({depositId});
      setLoadings(false);
      addToast(result.message, {appearance: "success",autoDismiss: true});
      handlegetdata();
      getUserDetails();
  ...
  const AcceptButtonTemplate = (row) => <Button onClick={() => handleApprove(row._id)} style={{background:"green",color:'#fff'}}>Approve</Button>
  const RejectButtonTemplate = (row) => <Button onClick={() => handleReject(row._id)} style={{background:"red",color:'#fff'}}>Reject</Button>
